...
function Signup({ onSwitchToLogin, onSuccess }) {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-4 relative overflow-hidden selection:bg-emerald-500 selection:text-slate-950 transition-colors duration-300 ${isDarkMode
      ...
      <div className={`absolute top-1/4 left-1/4 w-96 h-96 rounded-full blur-[140px] pointer-events-none transition-opacity duration-300 ${isDarkMode ? 'bg-emerald-500/10' : 'bg-emerald-500/20'
        }`}></div>
      <div className={`absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full blur-[140px] pointer-events-none transition-opacity duration-300 ${isDarkMode ? 'bg-cyan-500/10' : 'bg-teal-500/20'
        }`}></div>

      {/* Main Card */}
      <div className={`w-full max-w-md px-6 py-6 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.1)] backdrop-blur-2xl relative z-10 transition-all duration-300 border ${isDarkMode
        ...
        <div className="relative">
          ...
          <div className="text-center mb-5">
            ...
            <h2 className={`text-2xl font-black tracking-tight bg-gradient-to-r bg-clip-text text-transparent ${isDarkMode ? 'from-white via-slate-100 to-emerald-300' : 'from-slate-900 via-slate-800 to-emerald-600'
              }`}>
              Create Account
            </h2>
            <p className={`text-xs mt-1 flex items-center justify-center gap-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>
              <Sparkles className="w-3 h-3 text-emerald-500 animate-pulse" /> Join ChatWave & experience real-time chat
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {/* Name Field */}
            <div>
              <label className={`block text-[11px] font-bold uppercase tracking-wider mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'
                }`}>
                Full Name
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full border rounded-xl px-4 py-2.5 pl-10 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 transition ${isDarkMode
