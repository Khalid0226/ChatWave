        ...
        <div className={`w-full h-full md:h-[88vh] md:max-w-2xl md:border md:rounded-2xl flex flex-col shadow-2xl overflow-hidden relative transition-colors duration-300 ${
          ...
          <div className={`h-16 px-4 md:px-6 flex items-center justify-between border-b shrink-0 backdrop-blur-xl transition-colors duration-300 ${
            ...
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-sm font-bold tracking-tight text-emerald-500">Communities</h3>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 pb-20 md:pb-6 custom-scrollbar">
            ...
            <div className={`flex items-center justify-between p-4 border rounded-2xl shadow-md transition-colors duration-300 ${
              ...
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center text-emerald-500 shadow-lg">
                  <Users className="w-6 h-6" />
                </div>
                <div>
                  <h4 className={`text-xs font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>New Community</h4>
            ...
            {/* Communities List */}
            <div>
              <p className="text-[11px] font-semibold text-emerald-500 uppercase tracking-wider mb-3 px-1">
                Your Communities
              </p>
              ...
              <div className="space-y-3">
                {communities.map((comm) => (
                  <div 
                    ...
                    className={`p-4 border rounded-2xl transition group cursor-pointer space-y-3 ${
                      isDarkMode 
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl border flex items-center justify-center text-emerald-500 font-bold text-sm shrink-0 ${
                          isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-100 border-slate-300'
                        }`}>
                          {comm.name.charAt(0)}
                        </div>
                        <div>
                          <h4 className={`text-xs font-bold transition ${isDarkMode ? 'text-white group-hover:text-emerald-500' : 'text-slate-900 group-hover:text-emerald-600'}`}>
                            {comm.name}
                          </h4>
                    ...
                    <p className={`text-[11px] pl-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                      {comm.description}
                    </p>

                    {/* Announcement Sub-item */}
                    <div className={`pt-2 border-t flex items-center justify-between text-[11px] transition ${
                      isDarkMode ? 'border-slate-800/60 text-slate-400 hover:text-white' : 'border-slate-200 text-slate-500 hover:text-slate-900'
                    }`}>
          ...
          {selectedCommunity && (
            <div className={`absolute inset-0 z-50 flex flex-col justify-between p-6 animate-fadeIn transition-colors duration-300 ${
              ...
              <div className={`flex items-center justify-between border-b pb-4 ${
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-emerald-500/20 border border-emerald-500/30 flex items-center justify-center text-emerald-500 font-bold text-base">
                    {selectedCommunity.name.charAt(0)}
                  </div>
                  <div>
                    <h4 className={`text-sm font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{selectedCommunity.name}</h4>
              ...
              <div className="flex-1 overflow-y-auto py-6 space-y-4 custom-scrollbar">
                ...
                <div className={`p-4 border rounded-2xl space-y-2 ${
                  ...
                  <p className={`text-xs pl-6 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                ...
                <div className="flex items-center justify-between pt-2 px-1">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1.5 text-xs text-emerald-500 hover:text-emerald-400 font-medium bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-xl transition cursor-pointer"
                  >
                ...
                {showAddMemberBox && (
                  <form onSubmit={handleAddMemberToCommunity} className={`p-3 border rounded-2xl flex gap-2 ${
                    ...
                    <input 
                      ...
                      className={`flex-1 border rounded-xl px-3 py-2 text-xs focus:border-emerald-500 focus:outline-none ${
                        isDarkMode ? 'bg-slate-950 border-slate-800 text-white placeholder-slate-600' : 'bg-white border-slate-300 text-slate-900 placeholder-slate-400'
                      }`}
                      autoFocus
                      required
                    />
                    <button 
                      type="submit"
                      className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs rounded-xl transition cursor-pointer shadow-sm"
                    >
                      Add
                    </button>
                  </form>
                ...
                <div className="space-y-2">
                  {(selectedCommunity.groupsList || ['General Discussion Group']).map((grp, idx) => (
                    <div key={idx} className={`p-4 border rounded-2xl flex items-center justify-between cursor-pointer transition ${
                      isDarkMode ? 'bg-slate-900/50 border-slate-800 hover:bg-slate-800/40' : 'bg-slate-50 border-slate-200 hover:bg-slate-100 shadow-sm'
                    }`}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h5 className={`text-xs font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{grp}</h5>
              ...
              <div className="text-center text-[10px] text-slate-400 uppercase tracking-wider font-medium pt-2">
            ...
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
              <div className={`w-full max-w-md border rounded-2xl p-5 space-y-4 shadow-2xl transition-colors duration-300 ${
                ...
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-emerald-500">Create New Community</h3>
                  <button onClick={() => setShowCreateModal(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-white cursor-pointer p-1">
                    ✕
                  </button>
                </div>
                ...
                <form onSubmit={handleCreateCommunity} className="space-y-4">
                  <div>
                    <label className={`text-[11px] font-semibold block mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>Community Name</label>
                    <input
                      type="text"
                      value={commName}
                  ...
                  <div>
                    <label className={`text-[11px] font-semibold block mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>Initial Members Count (Optional)</label>
                    <input
                      type="number"
                      value={memberInput}
                  ...
                  <div>
                    <label className={`text-[11px] font-semibold block mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>Description</label>
                    <textarea
                      rows="3"
                      value={commDesc}
