...
function ChatDashboard({ user, onLogout }) {
  ...
  // Socket setup / User registration on mount
  useEffect(() => {
    if (user?._id || user?.id) {
  ...
  useEffect(() => {
    const handleReceiveMessage = (data) => {
      // Agar incoming message active chat wale user se hai tabhi state me append karo
      if (selectedChat && (data.sender === selectedChat._id || data.sender?._id === selectedChat._id)) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (selectedFile) {
        // Backend me upload.single('file') hona chahiye
      ...
      if (response.status === 201 || response.status === 200) {
        const sentMsg = response.data.newMessage || response.data;
      ...
      alert(error.response?.data?.message || "Message bhejte waqt error aaya.");
    }
  };
  ...
  return (
    <div className={`h-[100dvh] w-screen flex flex-col md:flex-row overflow-hidden selection:bg-emerald-500 selection:text-white relative transition-colors duration-300 ${isDarkMode ? 'bg-[#050811] text-white' : 'bg-slate-50 text-slate-900'
      }`}>

      {/* Desktop Rail / Sidebar */}
      <div className={`hidden md:flex w-16 border-r flex-col items-center py-4 justify-between z-35 shrink-0 transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 border-slate-800/80' : 'bg-white border-slate-200'
        }`}>
        <div className="flex flex-col items-center gap-4">
          ...
          <button
            ...
            className={`p-3 rounded-xl transition cursor-pointer ${isDarkMode ? 'text-slate-400 hover:text-white hover:bg-slate-800/50' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'
              }`}
            title="Status"
          >
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
        ...
        {/* Chats List Sidebar */}
        <div className={`w-full md:w-80 lg:w-96 border-r flex flex-col backdrop-blur-xl transition-colors duration-300 ${isDarkMode ? 'bg-slate-900/40 border-slate-800/80' : 'bg-white/70 border-slate-200'
          } ${selectedChat ? 'hidden md:flex' : 'flex'} flex-1 md:flex-initial pb-16 md:pb-0`}>

          {/* Chats Header with Add Contact Button */}
          <div className={`p-4 border-b flex items-center justify-between relative transition-colors duration-300 ${isDarkMode ? 'border-slate-800/80' : 'border-slate-200'
            }`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative md:hidden">
                <button
                  ...
                  className={`p-2 rounded-xl transition cursor-pointer ${isDarkMode ? 'text-slate-400 hover:text-white hover:bg-slate-800/50' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'
                    }`}
                >
                ...
                {showMenuDropdown && (
                  <div className={`absolute right-0 mt-2 w-44 border rounded-2xl shadow-2xl py-2 z-50 text-xs transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-200' : 'bg-white border-slate-200 text-slate-700'
                    }`}>
                    <button
                      onClick={() => { navigate('/profile'); setShowMenuDropdown(false); }}
                      className={`w-full text-left px-4 py-2.5 flex items-center gap-2 cursor-pointer ${isDarkMode ? 'hover:bg-slate-800' : 'hover:bg-slate-100'
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-red-400 flex items-center gap-2 font-medium cursor-pointer ${isDarkMode ? 'hover:bg-slate-800' : 'hover:bg-slate-100'
                        }`}
                    >
              ...
              <button
                onClick={handleLogout}
                title="Logout"
                className={`hidden md:block p-2 rounded-xl transition cursor-pointer ${isDarkMode ? 'text-slate-400 hover:text-red-400 hover:bg-slate-800/50' : 'text-slate-500 hover:text-red-600 hover:bg-slate-100'
                  }`}
              >
              ...
              contacts.map((contact) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="relative">
                      <div className={`w-11 h-11 rounded-2xl bg-gradient-to-tr flex items-center justify-center font-bold text-emerald-500 border overflow-hidden ${isDarkMode ? 'from-slate-800 to-slate-700 border-slate-700' : 'from-slate-100 to-slate-200 border-slate-300'
                        }`}>
                        {contact.avatar ? (
        ...
        {/* Main Chat Area */}
        <div className={`flex-1 flex flex-col relative transition-colors duration-300 ${isDarkMode ? 'bg-slate-950/40' : 'bg-slate-50/50'
          } ${selectedChat ? 'flex' : 'hidden md:flex'}`}>

          {selectedChat ? (
            <>
              {/* Chat Header */}
              <div className={`h-16 border-b px-4 md:px-6 flex items-center justify-between relative z-30 backdrop-blur-xl transition-colors duration-300 ${isDarkMode ? 'border-slate-800/80 bg-slate-950/80' : 'border-slate-200 bg-white/90'
                }`}>
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setSelectedChat(null)}
                    className={`md:hidden p-1.5 rounded-lg transition cursor-pointer ${isDarkMode ? 'text-slate-400 hover:text-white hover:bg-slate-800/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
                      }`}
                  >
                  ...
                  <div className="relative">
                    <div className={`w-10 h-10 rounded-2xl bg-gradient-to-tr flex items-center justify-center font-bold text-emerald-500 border overflow-hidden ${isDarkMode ? 'from-slate-800 to-slate-700 border-slate-700' : 'from-slate-100 to-slate-200 border-slate-300'
                      }`}>
                      {selectedChat.avatar ? (
                ...
                <div className="flex items-center gap-1 md:gap-2 text-slate-400 relative">
                  ...
                  <div className="relative">
                    ...
                    {showChatMenu && (
                      <div className={`absolute right-0 top-10 w-44 border rounded-2xl shadow-2xl py-2 z-50 text-xs transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-200' : 'bg-white border-slate-200 text-slate-700'
                        }`}>
                        <button
                          ...
                          <div className="mb-1.5">
                            {isImage ? (
                              <img
                                src={fileUrl}
                                alt="attachment"
                                className="max-w-full h-auto rounded-xl max-h-48 object-cover cursor-pointer"
              ...
              {/* Message Input Box */}
              <div className={`p-3 md:p-4 border-t backdrop-blur-xl z-10 relative transition-colors duration-300 ${isDarkMode ? 'border-slate-800/80' : 'border-slate-200 bg-white/80'
                }`}>
                {selectedFile && (
                  <div className={`flex items-center justify-between border px-3 py-2 rounded-xl mb-2 text-xs ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-300' : 'bg-slate-100 border-slate-200 text-slate-700'
                    }`}>
                ...
                <form onSubmit={handleSendMessage} className="flex items-center gap-2 md:gap-3">
                  <div className={`flex-1 relative flex items-center border rounded-xl px-3 transition ${isDarkMode
                    ...
                    <input
                      ...
                      className={`w-full bg-transparent px-3 py-3 text-xs placeholder-slate-400 focus:outline-none ${isDarkMode ? 'text-white' : 'text-slate-900'
                        }`}
                    />
