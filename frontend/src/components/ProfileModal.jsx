  ...
  return (
    <div className={`h-[100dvh] w-screen flex flex-col justify-between overflow-hidden relative selection:bg-emerald-500 selection:text-slate-950 transition-colors duration-300 ${
      ...
      <div className="flex-1 flex items-center justify-center p-0 md:p-6 overflow-hidden">
        ...
        {/* Main Card Container */}
        <div className={`w-full h-full md:h-[88vh] md:max-w-2xl md:border md:rounded-2xl flex flex-col shadow-2xl overflow-hidden transition-colors duration-300 ${
          isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
        }`}>
          
          {/* Header */}
          <div className={`h-16 px-4 md:px-6 flex items-center gap-4 border-b shrink-0 backdrop-blur-xl transition-colors duration-300 ${
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pb-20 md:pb-6 custom-scrollbar">
            ...
            {/* Avatar Section */}
            <div className={`flex flex-col items-center py-6 border-b transition-colors duration-300 ${
              isDarkMode ? 'bg-slate-950/30 border-slate-800/60' : 'bg-slate-50/50 border-slate-200'
            }`}>
            ...
            {/* Name Section */}
            <div className={`px-6 py-4 border-y transition-colors duration-300 ${
              isDarkMode ? 'bg-slate-950/40 border-slate-800/60' : 'bg-slate-50/60 border-slate-200'
            }`}>
              ...
              <div className="flex items-center justify-between">
                {isEditingName ? (
                  <input 
                    ...
                    className={`w-full border border-emerald-500 rounded-lg px-3 py-1.5 text-xs focus:outline-none ${
                      isDarkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900 shadow-sm'
                    }`}
                    autoFocus
                  />
                ) : (
            ...
            {/* Phone Section */}
            <div className={`px-6 py-4 border-y flex items-center justify-between transition-colors duration-300 ${
              isDarkMode ? 'bg-slate-950/40 border-slate-800/60' : 'bg-slate-50/60 border-slate-200'
            }`}>
              <div>
            ...
            <div className="px-4 space-y-1 pt-2">
              <div className={`flex items-center gap-4 px-4 py-3 rounded-xl cursor-pointer transition text-xs ${
                isDarkMode ? 'text-slate-300 hover:bg-slate-800/40' : 'text-slate-700 hover:bg-slate-100'
              }`}>
